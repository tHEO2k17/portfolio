...
module.exports = function requireAll(rootPath, config){
	config = config || {};
	...
	function requireContent(stats){
		if (stats.isFile()){
			var key = path.parse(rootPath).name;
			var object = {};
		...
		function ignoreIndexSibling(meta){
			var pathHasIndex = meta.filter(hasIndex)
				.reduce(indexByPath, {});

			return meta.filter(notIndexSibbling);
		...
		function serialize(meta){
			var object = meta.reduce(toObject, {});
			if (config.flatten)
				return object;
	...
	function normalizeRequirePath(requirePath){
		var relativeRequirePath = path.join(path.relative(__dirname, process.cwd()), requirePath);
